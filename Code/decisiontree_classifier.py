...
def right_left_context(sent, target_word):
    ...
    for index, tup in enumerate(i):
        word = tup[0]
        if word == target_word:
            left_context_word = i[index-1]
            if index+1 < len(i):
                right_context_word = i[index+1]
            else:
                right_context_word = ("(N/A)", "none")
            if 0 <= index-2:
                left_two_context_word = i[index-2]
            else:
...
parser.add_argument('target_word_1') # First candidate from the confusion set
parser.add_argument('target_word_2') # Second candidate from the confusion set
...
with open(args.filename, encoding='utf8') as data_file: # The datafile contains all sentence examples of the category in consideration
    ...
    target_word_1 = args.target_word_1
    target_word_2 = args.target_word_2
    sent_list_1 = sent_list(sentences, target_word_1)
    sent_list_2 = sent_list(sentences, target_word_2)
    data_matrix_1 = np.zeros([1,12])
    data_matrix_2 = np.zeros([1,12])
...
graph.write_png('tree_test.png') # Outputs a visualized tree graph
...
plt.title("Feature importance for " + str(target_word_1) + " and " + str(target_word_2))
plt.yticks(y_pos, feature_names)
plt.show() # Outputs a bar chart showing the importance of each feature